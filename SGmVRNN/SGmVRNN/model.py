...
class EncX(nn.Module):
    '''
    Input: x
        shape: [batch_size, T, 1, n, w]   (after DataLoader batching)
    Output: x_hat
        shape: [batch_size, T, conv_dim]
    Describe: CNN encoder that compresses n KPIs into conv_dim, works for any n
    '''
    def __init__(self, enc_dim, enc='CNN', n=38, w=1, T=20):
        ...
        if self.enc == 'CNN' and self.w == 1:
            # Generic 3-layer CNN: kernel=3, stride=2, padding=1
            # n → ceil(n/2) → ceil(n/4) → ceil(n/8) = s_d
            k, s, p = 3, 2, 1
            s_d = (n + 2*p - k) // s + 1  # after layer 1
            s_d = (s_d + 2*p - k) // s + 1  # after layer 2
            s_d = (s_d + 2*p - k) // s + 1  # after layer 3
            if s_d < 1:
                raise ValueError(f'n={n} is too small for 3 conv layers (s_d={s_d})')

            self.cd = [32, s_d]
            self.conv = nn.Sequential(
                ConvUnit1d(1, 8, kernel=k, stride=s, padding=p),
                ConvUnit1d(8, 16, kernel=k, stride=s, padding=p),
                ConvUnit1d(16, 32, kernel=k, stride=s, padding=p),
            )
    ...
    def enc_x(self, x):
        if self.enc == 'CNN' and self.w == 1:
            # x from DataLoader: [batch, T, 1, n, 1]
            # Conv1d needs (B*C, L), so merge batch and T dimensions
            BT = x.size(0) * x.size(1)
            x = x.squeeze(-1)                          # [batch, T, 1, n]
            x = x.contiguous().view(BT, 1, self.n)     # [batch*T, 1, n]
            x = self.conv(x)                           # [batch*T, 32, s_d]
            x = x.view(BT, -1)                         # [batch*T, 32*s_d]
            x = self.conv_fc(x)                        # [batch*T, conv_dim]
            x = x.view(-1, self.T, self.conv_dim)      # [batch, T, conv_dim]
        else:
            raise ValueError('Unknown encoder: {}'.format(self.enc))
        return x
...
class DecX(nn.Module):
    ...
    Describe: CNN decoder that reconstructs n KPIs from latent, works for any n
    '''
    def __init__(self, enc_dim, dec_init_dim, dec='CNN', n=38, w=1, T=20):
        ...
        if self.dec == 'CNN' and self.w == 1:
            # Same encoder params: kernel=3, stride=2, padding=1
            k, s, p = 3, 2, 1
            s_d = (n + 2*p - k) // s + 1
            s_d = (s_d + 2*p - k) // s + 1
            s_d = (s_d + 2*p - k) // s + 1
            if s_d < 1:
                raise ValueError(f'n={n} is too small for 3 deconv layers (s_d={s_d})')

            # Decoder output size: s_d → 2*s_d-1 → 4*s_d-3 → 8*s_d-7
            ...
            self.deconv_mu = nn.Sequential(
                ConvUnitTranspose1d(32, 16, kernel=k, stride=s, padding=p),
                ConvUnitTranspose1d(16, 8, kernel=k, stride=s, padding=p),
                ConvUnitTranspose1d(8, 1, kernel=k, stride=s, padding=p,
    ...
    def _decode(self, x, deconv_fc, deconv):
        if self.dec == 'CNN' and self.w == 1:
            x = deconv_fc(x)                              # [batch, 32*s_d]
            x = x.view(-1, self.cd[0], self.cd[1])        # [batch, 32, s_d]
            x = deconv(x)                                 # [batch, 1, decoder_out]
            # Interpolate to exactly n if needed
            if self.decoder_out != self.n:
                x = F.interpolate(x, size=self.n, mode='linear', align_corners=False)
            x = x.view(-1, 1, 1, self.n, self.w)          # [batch, 1, 1, n, 1]
        else:
            raise ValueError('Unknown decoder: {}'.format(self.dec))
        return x
